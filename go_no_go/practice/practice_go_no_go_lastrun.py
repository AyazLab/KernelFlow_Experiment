...
expName = 'practice_go_no_go'  # from the Builder filename that created this script
expInfo = {'participant': 'practice', 'session': '001'}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\zackg\\OneDrive\\Ayaz Lab\\KernelFlow_PsychoPy\\go_no_go\\practice\\practice_go_no_go_lastrun.py',
...
conditions_csv = os.path.join(os.getcwd(), "practice_go_no_go_conditions.csv")
...
practice_loop = data.TrialHandler(nReps=1.0, method='sequential', 
    ...
    trialList=data.importConditions('practice_go_no_go_conditions.xlsx'),
...
for thisPractice_loop in practice_loop:
    ...
    practice_block = data.TrialHandler(nReps=1.0, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions(task_order),
    ...
    for thisPractice_block in practice_block:
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if inter_stim_plus.status == STARTED:
                ...
                if tThisFlipGlobal > inter_stim_plus.tStartRefresh + 5-frameTolerance:
        ...
        for thisComponent in inter_stim_intervalComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        practice_block.addData('inter_stim_plus.started', inter_stim_plus.tStartRefresh)
        practice_block.addData('inter_stim_plus.stopped', inter_stim_plus.tStopRefresh)
        ...
        # keep track of which components have finished
        practice_trialComponents = [go_resp, go_plus, go_image]
        ...
        practice_block.addData('go_resp.keys',go_resp.keys)
        practice_block.addData('go_resp.corr', go_resp.corr)
        if go_resp.keys != None:  # we had a response
            practice_block.addData('go_resp.rt', go_resp.rt)
        practice_block.addData('go_resp.started', go_resp.tStartRefresh)
        practice_block.addData('go_resp.stopped', go_resp.tStopRefresh)
        practice_block.addData('go_image.started', go_image.tStartRefresh)
        practice_block.addData('go_image.stopped', go_image.tStopRefresh)
        thisExp.nextEntry()
