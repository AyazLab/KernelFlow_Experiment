...
response_interval_text = visual.TextStim(win=win, name='response_interval_text',
    ...
    languageStyle='LTR',
    depth=0.0);
subject_resp = keyboard.Keyboard()
...
for thisGNG_loop in GNG_loop:
    ...
    for thisGo_block in go_block:
        ...
        subject_resp.keys = []
        subject_resp.rt = []
        _subject_resp_allKeys = []
        # keep track of which components have finished
        response_intervalComponents = [response_interval_text, subject_resp]
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            # *subject_resp* updates
            waitOnFlip = False
            if subject_resp.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                subject_resp.frameNStart = frameN  # exact frame index
                subject_resp.tStart = t  # local t and not account for scr refresh
                subject_resp.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(subject_resp, 'tStartRefresh')  # time at next scr refresh
                subject_resp.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(subject_resp.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(subject_resp.clearEvents, eventType='keyboard')  # clear events on next screen flip
            if subject_resp.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > subject_resp.tStartRefresh + 3-frameTolerance:
                    # keep track of stop time/frame for later
                    subject_resp.tStop = t  # not accounting for scr refresh
                    subject_resp.frameNStop = frameN  # exact frame index
                    win.timeOnFlip(subject_resp, 'tStopRefresh')  # time at next scr refresh
                    subject_resp.status = FINISHED
            if subject_resp.status == STARTED and not waitOnFlip:
                theseKeys = subject_resp.getKeys(keyList=['space'], waitRelease=False)
                _subject_resp_allKeys.extend(theseKeys)
                if len(_subject_resp_allKeys):
                    subject_resp.keys = _subject_resp_allKeys[-1].name  # just the last key pressed
                    subject_resp.rt = _subject_resp_allKeys[-1].rt
                    # was this correct?
                    if (subject_resp.keys == str(match)) or (subject_resp.keys == match):
                        subject_resp.corr = 1
                    else:
                        subject_resp.corr = 0
                    # a response ends the routine
        ...
        # check responses
        if subject_resp.keys in ['', [], None]:  # No response was made
            subject_resp.keys = None
            # was no response the correct answer?!
            if str(match).lower() == 'none':
               subject_resp.corr = 1;  # correct non-response
            else:
               subject_resp.corr = 0;  # failed to respond (incorrectly)
        # store data for go_block (TrialHandler)
        go_block.addData('subject_resp.keys',subject_resp.keys)
        go_block.addData('subject_resp.corr', subject_resp.corr)
        if subject_resp.keys != None:  # we had a response
            go_block.addData('subject_resp.rt', subject_resp.rt)
        go_block.addData('subject_resp.started', subject_resp.tStartRefresh)
        go_block.addData('subject_resp.stopped', subject_resp.tStopRefresh)
        thisExp.nextEntry()
    ...
    for thisGNG_block in GNG_block:
        ...
        GNG_block.addData('subject_resp.keys',subject_resp.keys)
        GNG_block.addData('subject_resp.corr', subject_resp.corr)
        if subject_resp.keys != None:  # we had a response
            GNG_block.addData('subject_resp.rt', subject_resp.rt)
        GNG_block.addData('subject_resp.started', subject_resp.tStartRefresh)
        GNG_block.addData('subject_resp.stopped', subject_resp.tStopRefresh)
        thisExp.nextEntry()
