...
while continueRoutine:
    ...
    waitOnFlip = False
    if demo_instructions_resp.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        ...
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(demo_instructions_resp.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(demo_instructions_resp.clearEvents, eventType='keyboard')  # clear events on next screen flip
    if demo_instructions_resp.status == STARTED and not waitOnFlip:
...
# check responses
if demo_instructions_resp.keys in ['', [], None]:  # No response was made
    demo_instructions_resp.keys = None
thisExp.addData('demo_instructions_resp.keys',demo_instructions_resp.keys)
if demo_instructions_resp.keys != None:  # we had a response
    thisExp.addData('demo_instructions_resp.rt', demo_instructions_resp.rt)
thisExp.addData('demo_instructions_resp.started', demo_instructions_resp.tStartRefresh)
thisExp.addData('demo_instructions_resp.stopped', demo_instructions_resp.tStopRefresh)
thisExp.nextEntry()
...
while continueRoutine:
    ...
    waitOnFlip = False
    if test1_instructions_resp.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        ...
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(test1_instructions_resp.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(test1_instructions_resp.clearEvents, eventType='keyboard')  # clear events on next screen flip
    if test1_instructions_resp.status == STARTED and not waitOnFlip:
...
for thisComponent in test1_card_instructionsComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
thisExp.addData('test1_instructions_text.started', test1_instructions_text.tStartRefresh)
thisExp.addData('test1_instructions_text.stopped', test1_instructions_text.tStopRefresh)
# check responses
if test1_instructions_resp.keys in ['', [], None]:  # No response was made
    test1_instructions_resp.keys = None
thisExp.addData('test1_instructions_resp.keys',test1_instructions_resp.keys)
if test1_instructions_resp.keys != None:  # we had a response
    thisExp.addData('test1_instructions_resp.rt', test1_instructions_resp.rt)
thisExp.addData('test1_instructions_resp.started', test1_instructions_resp.tStartRefresh)
thisExp.addData('test1_instructions_resp.stopped', test1_instructions_resp.tStopRefresh)
thisExp.nextEntry()
...
while continueRoutine:
    ...
    waitOnFlip = False
    if test2_instructions_resp.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        ...
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(test2_instructions_resp.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(test2_instructions_resp.clearEvents, eventType='keyboard')  # clear events on next screen flip
    if test2_instructions_resp.status == STARTED and not waitOnFlip:
...
for thisComponent in test2_card_instructionsComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
thisExp.addData('test2_instructions_text.started', test2_instructions_text.tStartRefresh)
thisExp.addData('test2_instructions_text.stopped', test2_instructions_text.tStopRefresh)
# check responses
if test2_instructions_resp.keys in ['', [], None]:  # No response was made
    test2_instructions_resp.keys = None
thisExp.addData('test2_instructions_resp.keys',test2_instructions_resp.keys)
if test2_instructions_resp.keys != None:  # we had a response
    thisExp.addData('test2_instructions_resp.rt', test2_instructions_resp.rt)
thisExp.addData('test2_instructions_resp.started', test2_instructions_resp.tStartRefresh)
thisExp.addData('test2_instructions_resp.stopped', test2_instructions_resp.tStopRefresh)
thisExp.nextEntry()
...
while continueRoutine:
    ...
    waitOnFlip = False
    if test3_instructions_resp.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        ...
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(test3_instructions_resp.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(test3_instructions_resp.clearEvents, eventType='keyboard')  # clear events on next screen flip
    if test3_instructions_resp.status == STARTED and not waitOnFlip:
...
for thisComponent in test3_card_instructionsComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
thisExp.addData('test3_instructions_text.started', test3_instructions_text.tStartRefresh)
thisExp.addData('test3_instructions_text.stopped', test3_instructions_text.tStopRefresh)
# check responses
if test3_instructions_resp.keys in ['', [], None]:  # No response was made
    test3_instructions_resp.keys = None
thisExp.addData('test3_instructions_resp.keys',test3_instructions_resp.keys)
if test3_instructions_resp.keys != None:  # we had a response
    thisExp.addData('test3_instructions_resp.rt', test3_instructions_resp.rt)
thisExp.addData('test3_instructions_resp.started', test3_instructions_resp.tStartRefresh)
thisExp.addData('test3_instructions_resp.stopped', test3_instructions_resp.tStopRefresh)
thisExp.nextEntry()
